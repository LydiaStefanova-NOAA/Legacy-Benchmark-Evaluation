...
c
      ...
      subroutine gridavgm(z,lon,lat,wt,lbms,gridv)
c
c.. computes accurate gaussian grid averages...with missing data
         real*4 z(lon,lat),wt(lat)
         logical*1 lbms(lon,lat)
c
         gridv=0.
         do j=1,lat
         xx=0.
...
11    CONTINUE
      ...
      SUBROUTINE gridavl(z,lon,lat,deg,lbms,gridv)
      dimension z(lon,lat),deg(lat)
      logical*1 lbms(lon,lat)
c
      gridv=0.
